...
export function cleanDescription(text: string | undefined | null): string {
    ...
    // 1. Eliminar marcadores de sección de Inmovilla (~~) y tildes sueltas (~)
    ...
    // 5. Eliminar otros caracteres de control o decorativos extraños
    cleaned = cleaned.replace(/[•▪♦✓►]/g, '');

    // 6. Normalizar espacios en blanco
    cleaned = cleaned.replace(/[ ]{2,}/g, ' ');

    // 7. Normalizar saltos de línea múltiples
