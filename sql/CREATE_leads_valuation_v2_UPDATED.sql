-- Tabla para leads de tasación con nuevo flujo (VERSIÓN ACTUALIZADA)
-- Incluye campos: habitaciones, banos, notas_adicionales
-- Fecha: 2026-03-04

-- Eliminar tabla si existe (solo para desarrollo, comentar en producción)
-- DROP TABLE IF EXISTS leads_valuation_v2 CASCADE;
...
CREATE TABLE IF NOT EXISTS leads_valuation_v2 (
  ...
  -- STEP 4: Detalles del inmueble (ACTUALIZADO)
...
DROP POLICY IF EXISTS "Inserts from anonymous" ON leads_valuation_v2;
DROP POLICY IF EXISTS "Only admins can view" ON leads_valuation_v2;
...
COMMENT ON TABLE leads_valuation_v2 IS 'Leads de valoración de propiedades - Versión 2 (actualizada)';
COMMENT ON COLUMN leads_valuation_v2.habitaciones IS 'Número de habitaciones proporcionado por el usuario';
COMMENT ON COLUMN leads_valuation_v2.banos IS 'Número de baños proporcionado por el usuario';
